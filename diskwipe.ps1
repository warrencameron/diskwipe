...
Function gather-Input-From-User() {
    ...
    foreach ($disk in $disks) {
        Write-Host "Disk ID:        $($disk.Number)" -BackgroundColor Black -ForegroundColor Red
    ...
    while (-Not ($disk_IDs -contains $global:disk_Id_To_Sanitize)) {
        ...
        if (-Not ($disk_IDs -contains $global:disk_Id_To_Sanitize)) {
            Clear
            Write-Host "An invalid Disk ID was entered. See valid Disk IDs below:" -BackgroundColor Black -ForegroundColor DarkYellow
            Write-Host
            Write-Host "----------------------------------- Disk Information -----------------------------------"
            Write-Host
     ...
     while (-Not (([int]$global:number_Of_Passes -ge 1) -and ([int]$global:number_Of_Passes -le 99))) {
        ...
        if (-Not (([int]$global:number_Of_Passes -ge 1) -and ([int]$global:number_Of_Passes -le 99))) {
            Clear
            Write-Host "An invalid number of passes was selected. Choose a number between 1-99:" -BackgroundColor Black -ForegroundColor DarkYellow
        }
    }
